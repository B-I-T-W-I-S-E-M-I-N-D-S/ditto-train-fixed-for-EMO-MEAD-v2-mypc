...
class MotionDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def ddim_sample(self, shape, cond_frame, cond, last_half=None, noise=None,
                    emotion_embed=None, **kwargs):
    ...
    def p_losses(self, x_start, cond_frame, cond, t, emotion_embed=None):
        noise = torch.randn_like(x_start)
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)

        # reconstruct
        x_recon = self.model(x_noisy, cond_frame, cond, t,
    ...
    def loss(self, x, cond_frame, cond, t_override=None, emotion_embed=None):
        batch_size = len(x)
        ...
        return self.p_losses(x, cond_frame, cond, t, emotion_embed=emotion_embed)

    def forward(self, x, cond_frame, cond, t_override=None, emotion_embed=None):
        return self.loss(x, cond_frame, cond, t_override, emotion_embed=emotion_embed)
